...
declare const ScavengeScreen: any;
declare const $: JQueryStatic; // jQuery global com tipagem forte

const storageService = new StorageService();

const scavengeOptions: Record<string, any[]> = {};
...
let haulCategory: number = 0;
let hours: number = 6;

let totalLoot: number;
let totalSpSwLoot: number;
let possibleLoot: number;
let spearRatio: number;
let swordRatio: number;
let totalHaul: number;
...
function scavenge(): void {
  ...
  const checkboxValues = JSON.parse(storageService.getItem("checkboxValues")) || {},
    $checkboxes = $("#scavTable :checkbox");
  $checkboxes.on("change", function () {
    $checkboxes.each(function () {
      checkboxValues[this.id] = (this as HTMLInputElement).checked;
    });
  ...
  $.each(checkboxValues, function (key, value) {
    $(`#${key as String}`).prop("checked", value);
  });

  unitAmounts.spears = parseInt(
    $(".units-entry-all[data-unit=spear]")
      .text()
      .match(/\((\d+)\)/)?.[1] ?? "0"
  );
  unitAmounts.swords = parseInt(
    $(".units-entry-all[data-unit=sword]")
      .text()
      .match(/\((\d+)\)/)?.[1] ?? "0"
  );
  unitAmounts.axes = parseInt(
    $(".units-entry-all[data-unit=axe]")
      .text()
      .match(/\((\d+)\)/)?.[1] ?? "0"
  );
  unitAmounts.lightC = parseInt(
    $(".units-entry-all[data-unit=light]")
      .text()
      .match(/\((\d+)\)/)?.[1] ?? "0"
  );
  unitAmounts.heavyC = parseInt(
    $(".units-entry-all[data-unit=heavy]")
      .text()
      .match(/\((\d+)\)/)?.[1] ?? "0"
  );
  unitAmounts.archer = isArcherUnitEnabled()
    ? parseInt(
        $(".units-entry-all[data-unit=archer]")
          .text()
          .match(/\((\d+)\)/)?.[1] ?? "0"
      )
    : 0;
  unitAmounts.marcher = isMountedArcherUnitEnabled()
    ? parseInt(
        $(".units-entry-all[data-unit=marcher]")
          .text()
          .match(/\((\d+)\)/)?.[1] ?? "0"
      )
    : 0;
  ...
  const lackadaisicalLooters: string = $(".title")[0].innerHTML;
  const humbleHaulers: string = $(".title")[1].innerHTML;
  const cleverCollectors: string = $(".title")[2].innerHTML;
  const greatGatherers: string = $(".title")[3].innerHTML;
...
function createInterface(): void {
  ...
  $("#hours").on("change", function () {
    const inputHours = parseInt($("#hours").val() as string);
...
function getScavTime(): number {
  let hoursTemp: number;
...
function isArcherUnitEnabled(): boolean {
  return game_data.units.includes("archer");
}

function isMountedArcherUnitEnabled(): boolean {
  return game_data.units.includes("marcher");
}

function createScavengeTable(): string {
...
function calculateHauls(): void {
  const duration_factor: number = ScavengeScreen?.village.options[1].base.duration_factor ?? 0;
  const duration_exponent: number = ScavengeScreen?.village.options[1].base.duration_exponent ?? 0;
  const duration_initial_seconds: number = ScavengeScreen?.village.options[1].base.duration_initial_seconds ?? 0;
  ...
  const hours: number = getScavTime()
  let time = hours * 3600;
...
function fillInTroops(option: string, availableUnits: Record<string, number>, button: JQuery<HTMLElement>): void {
...
function emptyTroops(option: string): void {
  scavengeOptions[option].forEach(units => {
    const type = units.type;
...
function findNextButton(option: string): JQuery<HTMLElement> | undefined {
...
function getAvailableUnits(): Record<string, number> {
  let availableUnits: Record<string, number> = {};

  $('.units-entry-all').each((i, e) => {
    const unitName: string = $(e).attr("data-unit") as string;
    const count: string = $(e).text().replace(/[()]/, '');
